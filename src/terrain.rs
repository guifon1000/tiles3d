...
pub fn terrain_mesh(
    ...
    for (_i, _j, _k, _corners) in subpixels.iter() {
        ...
        for (lon, lat) in corners.iter() {
            ...
            let corner_ijk = planisphere.geo_to_subpixel(*lon, *lat);
            let corner_elevation = planisphere.get_rgba_at_subpixel(corner_ijk.0, corner_ijk.1, corner_ijk.2).0;
            vertices.push([x as f32, 0.*corner_elevation as f32, y as f32]);
        }
